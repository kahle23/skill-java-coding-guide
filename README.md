...
| [code-review-workflow](./code-review-workflow) | 代码审查工作流（多语言通用，同时支持逻辑BUG和代码BUG，含分级报告模板） | ✅ 已完成 |

> 技能之间相互独立，可按需选用；也鼓励把它们组合使用，覆盖从需求到设计到编码到测试到审查的完整链路。
...
└── code-review-workflow/     # 代码审查工作流（多语言通用，逻辑BUG + 代码BUG）
```
...
| "帮我 review 这段代码 / 审查代码" | code-review-workflow: 01 → 02（按语言选专项）→ 03 |
| "看看这段代码有没有问题" | code-review-workflow: 01 → 02 → 03 |
| "这段代码逻辑对不对" | code-review-workflow: 01 → 04（获取业务背景）→ 02（逻辑BUG专项） |
| "代码走查 / 代码评审" | code-review-workflow: 01 → 02 → 03 |
...
- [x] **code-review-workflow** — 代码审查工作流（多语言通用，逻辑BUG + 代码BUG，分级报告）
